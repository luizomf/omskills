...
When the user cannot recall the applicable user-only skill without inspecting or searching the list, recommend a user-only **Router Skill**; create it only when the accepted scope authorizes that addition. Its context pointers must name each target and the condition for loading it. This is a skill-selection role only; it never owns Mission Ticket identities, topology or Ticket-coordinator dispatch. Mechanical Mission routing belongs to `dispatch-tickets` when used.
