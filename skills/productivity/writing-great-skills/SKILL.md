...
A loaded skill may compose a user-only skill through a **context pointer** that links directly to its file and states the condition for reading it. Use relative Markdown paths for bundled and cross-skill references. Give each composing skill an executable resolution step: `python3 -c 'from pathlib import Path; import sys; print((Path(sys.argv[1]).resolve().parent / sys.argv[2]).resolve())' '<loaded-file-path>' '<relative-link>'`. Substitute the actual loaded file and link, run it, then read the printed path. Resolve the source symlink before applying `..`; lexical normalization against the installation path is not physical resolution. Read the linked file even when its skill is absent from the discovery list; report unavailability only after checking the target. When forwarding a pointer to a fresh agent, resolve it first and include the resulting path in the prompt. Keep machine-specific absolute paths out of distributed files.
...
When the user cannot recall the applicable user-only skill without inspecting or searching the list, recommend a user-only **Router Skill**; create it only when the accepted scope authorizes that addition. Its context pointers must link each target and state the condition for reading it. This is a skill-selection role only; it never owns Mission Ticket identities, topology or Ticket-coordinator dispatch. Mechanical Mission routing belongs to `dispatch-tickets` when used.

For skills that route Ticket execution, distinguish Direct Assisted work from Mission topology. An untracked request or exactly one selected Ticket may remain with the conversational responsible agent when the maintainer is available; readiness and Prompt Audit are not required there by default. For Unattended execution, readiness and a current Prompt Audit gate establish eligibility but never Mission authorization. When selecting the dispatcher route, supply its complete finite pre-resolved Mission plan to `dispatch-tickets`; `implement` may compose exactly one selected Ticket as a one-item plan through that dispatcher. For smaller Mission work, an authorized human/invoker or context-rich parent may directly dispatch one fresh isolated `orchestrate` coordinator. It checks selected-Ticket authority, live gates and actual capabilities, not caller provenance or role/depth assertions; dispatcher mechanical boundaries remain mandatory when used. Neither skill discovery nor a ready-work query selects Mission work.
