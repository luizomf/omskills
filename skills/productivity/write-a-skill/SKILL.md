...
2. **Establish requirements and authority.** Resolve all of these questions:
   ...
   - If it handles Tickets, does it preserve the boundary between readiness and explicit Mission authorization, route one selected Ticket directly to `orchestrate`, and route that same one exact identity to `dispatch-tickets` only when a responsive root is required?
