...
Load installed `model-routing` before selecting models for the interpreter and reviewer. Honor explicitly selected audit routes; otherwise classify the actual comprehension/comparison task within authorized scope. Apply routing through lifecycle fields only, without adding coordinator assessments or routing rationale to either pass's restricted semantic inputs. A lower-tier model understanding a prompt is evidence for that run, not proof that every other model will interpret it identically.
