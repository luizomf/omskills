...
Route work by consequence, then use the matching model from the current vendor. This file is the single source of truth for role-to-model selection. Anthropic always uses Opus and OpenAI always uses Sol; vary only the effort by route. Google keeps route-specific models.
...
| **Coordinator** | Selecting and adjudicating prepared work, correcting candidates, and integrating results | `claude-opus-5` | `gpt-5.6-sol` | `gemini-3.6-flash` | `high` |
| **Writer** | Implementing a prepared issue and running its verification | `claude-opus-5` | `gpt-5.6-sol` | `gemini-3.6-flash` | `medium` |
...
| **Audit reviewer** | Comparing reconstructed intent with authoritative intent | `claude-opus-5` | `gpt-5.6-sol` | `gemini-3.6-flash` | `high` |
| **Utility** | Mechanical reading, extraction, classification, transformation, and focused summaries | `claude-opus-5` | `gpt-5.6-sol` | `gemini-3.1-flash-lite` | `low` |
...
5. Do not substitute a lower-capability model when the routed model is unavailable. For delegated work, report the unavailable route instead of dispatching unless the user explicitly approves another model.
6. If the harness cannot select the active model, continue with the active model and report the deviation; do not interrupt the work solely to enforce this policy.
