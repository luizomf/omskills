...
Follow the `tmux-worker` workflow to launch one visible, interactive Pi worker with a clean conversation context. Give it a self-contained architecture scan brief, a findings artifact path, and the callback target.
...
The worker produces findings only, not the final HTML report. After its callback arrives, read the findings artifact, adjudicate every candidate by checking its cited code paths against the repository and authoritative docs, and continue with the report process in [SKILL.md](SKILL.md).
