...
- **Create an issue noninteractively:** `glab issue create --repo https://<host>/<namespace>/<project> --title <title> --description <description> --yes`. Both title and description are required; `--yes` suppresses submission confirmation, and supplying both values prevents the editor prompt.
- **Read an issue:** `glab issue view <iid> --repo https://<host>/<namespace>/<project> -F json`.
- **List issues as JSON:** `glab issue list --repo https://<host>/<namespace>/<project> -O json --per-page 100 --page <page>`. Iterate pages until a page contains fewer than 100 records. Use `-O json`, not `-F json`, because `-F` selects the human output format for `glab issue list`. Retain `author.username`, `created_at`, and `updated_at` until oldest-first and reporter-activity decisions are complete.
...
- **Comment / label / close:** use `glab mr note --message`, `glab mr update --label` / `--unlabel`, and `glab mr close`, always with the explicit `--repo` target.
...
- **Create a map noninteractively:** `glab issue create --repo https://<host>/<namespace>/<project> --title <title> --description <map-description> --label wayfinder:map --yes`.
- **Create a child noninteractively:** make `Part of #<map-iid>` the first line of the complete description, then run `glab issue create --repo https://<host>/<namespace>/<project> --title <title> --description <ticket-description> --label wayfinder:<type> --yes`. Create all children before adding blocker links.
...
- **Resolve the authenticated username:** `glab api --hostname <host> user --jq '.username'`.
