...
When web retrieval is needed, the worker uses `codex_search` with its default mini model to collect a source packet, then performs the actual analysis and produces the selected compact response or final research artifact itself. Run retrieval read-only and, in artifact mode, keep its output separate from the final report, for example:
