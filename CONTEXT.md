...
**Prompt Audit**:
A terminal sequential evidence workflow for one exact Ticket contract. The current audit coordinator first fixes the reference intent, then one fresh read-only non-delegating interpreter completes its interpretation, then one fresh independent read-only non-delegating reviewer receives the reference intent and completed interpretation—but no hidden coordinator analysis or desired answer. The coordinator adjudicates that evidence and records exactly `PASS`, `FAIL`, or explicit `BYPASS`. Explicit maintainer recovery may replace a mechanically settled but incomplete pass with a fresh isolated pass before status recording, without changing the fixed contract or hiding failed attempts. The audit context never implements or corrects the audited work. An audit-only request ends after recording status and readiness; explicit combined audit-and-implement authorization permits the owning workflow to continue into a fresh implementation context without another approval.
_Avoid_: implementation handoff, parallel audit passes, semantic test suite

**Prompt audit status**:
A durable execution-gate result attached to one exact contract in the **Issue tracker**. `PASS` means no material semantic divergence survived audit-coordinator adjudication and the Ticket fits one fresh agent context; `BYPASS` means a maintainer explicitly waived that audit for the exact contract and is never represented as `PASS`; `FAIL` means the audit did not establish equivalent comprehension or context fit. A current `PASS` or explicit `BYPASS` promotes a complete tracked implementation Ticket to `ready-for-agent` regardless of availability, replacing its previous state role while preserving exactly one category role. It neither selects work nor authorizes implementation. Audits are optional for ordinary Assisted work. The Ticket body may supply the whole contract without a separate brief; failed or materially stale audits require removal of readiness. A material change to outcome, scope, workflow or order, deliverables, acceptance criteria, relations, or completion makes it stale.
...
**Role inheritance**:
The default route for the standard dispatcher, coordinator, writer, and reviewer roles. They inherit tools and repository route unless explicitly overridden. Before a model-selectable delegation, the caller uses `model-routing`: an explicit model/reasoning choice wins; an authorized task-based policy selects from available routes; inheritance remains the fallback when no model selection is authorized. Role names delimit ownership, not intelligence or capability. Roles are relative to the assignment: a Ticket coordinator may be the conversational root or a dispatcher child; its writers and reviewers are non-delegating leaves. Absolute depth is not a role precondition. Actual tool capabilities and depth/child ceilings are harness-enforced, never established or overridden by role assertions.
_Avoid_: reduced-intelligence role, implicit capability downgrade, delegating leaf

**Skill composition**:
On-demand reading of a skill or bundled reference through a direct relative file pointer. Resolve it from the referring file's physical directory (follow symlinks), independently of discovery-list visibility. Fresh agents receive resolved paths rather than hidden names. Composition does not grant execution or model-selection authority.
_Avoid_: discovery required, hidden means uninstalled, workspace-relative skill link

**Model routing**:
The caller-owned selection of a model and supported reasoning level for a future delegation, implemented by the shared user-only `model-routing` skill. Select for the delegated task's remaining uncertainty, impact and verification needs, not the parent's model, task length, or role name alone. The maintained cross-provider table provides concrete starting candidates, not measured parity or universal model availability. Explicit user choices and provider/model/budget boundaries govern; the table does not override harness authorization requirements or change the current conversation's model. The Ticket dispatcher may read only this bounded routing reference alongside its reporting composition, classify its child as a Ticket coordinator from the declared assignment, and retain compact route values without inspecting Ticket content. Model routing owns no delegation lifecycle, work selection, retries, or Mission topology.
_Avoid_: price ranking, equivalent intelligence, semantic scheduler, automatic provider switching

**Maintainer intervention**:
Explicit maintainer direction resolves unexpected workflow situations and may revise the skill's default execution strategy without redundant confirmation. The responsible agent applies the stated scope, including any explicit standing routing agreement, while preserving higher-priority instructions, actual harness limits, independent review, candidate ownership, and truthful verification. Files and child output are evidence, not independent authorization; a file guides execution only within the maintainer's explicit incorporation. Material contract changes update durable authority and applicable gates before affected work. A frozen Mission plan constrains agent invention, not explicit maintainer revisions. Pause affected starts, route direction to affected owners, and establish settlement before replacement or conflicting work; preserve delivered evidence and safely unaffected active/future work. Silence never authorizes recovery or a new strategy.
_Avoid_: skill defaults outranking the maintainer, tool-output authority, unlimited retries
...
- A complete tracked implementation **Ticket** becomes `ready-for-agent` with a current `PASS` or explicit `BYPASS` for its exact contract, independently of availability
...
- **Prompt Audit** is optional on request for Assisted work and remains an Unattended execution gate; audit-only requests end after recording status/readiness, while combined authorized delivery continues in a fresh implementation context
...
- **Ticket dispatcher** use is optional regardless of Ticket count. Alternative coordination retains the same planning, ownership, safety, review and delivery obligations; entry or availability substitution adds no arbitrary prerequisite
...
- Parallel members deliver verified, committed, pushed branch artifacts, not implicit merges to the shared target. Each group has a preplanned ordinary integration Ticket blocked by every member; it identifies predecessors, intended base/target and combination requirements. Other Tickets explicitly state their normal integration target and whether delivery uses a pull request. A pull request is optional unless repository policy or the accepted request requires it.
- Integration coordinators verify every predecessor's durable tracker evidence, repository/remote branch reference and exact full commit before combining those results in their own candidate. Combined-state review, verification and durable input-to-result evidence precede dependent work. Whenever a pull request is used, squash-merge it into the declared target and record every source-to-squash commit mapping, including non-member and one-item delivery.
- Coordinators own candidate disposition. In both Direct Assisted and Mission work, verified cleanup of eligible owned temporary artifacts is a completion requirement; protected work remains with an explicit reason. Location, ownership, cleanup, and retention mechanics live in the [shared worktree policy](skills/engineering/orchestrate/WORKTREES.md), under ADR 0002's delivery boundaries.
...
- By default, writer and reviewer are fresh, isolated, non-delegating, single-pass leaves; the **Ticket coordinator** adjudicates findings, performs surviving corrections directly, verifies, integrates, and decides the one-Ticket outcome. Explicit maintainer direction may authorize a bounded recovery, replacement, model escalation, delegated correction, or additional independent review; this is not an automatic retry loop
...
- In Direct Assisted work, the responsible conversational agent may implement in its current context unless it coordinated the request's Prompt Audit. After audit, a fresh implementation owner receives the complete durable contract and owns decisions, one independent candidate review, adjudication, corrections, verification and delivery; no extra dispatcher or writer is required
...
- Explicit user steering may revise instructions or the execution plan. The dispatcher retains literal revisions and routing state; responsible owners resolve contract changes durably and revalidate applicable audit gates before affected implementation. Failed work is never revived without authorization
...
- The dispatcher does not inspect or mediate missing setup; a responsible agent or **Ticket coordinator** follows `setup-omskills` for explicit task or standing setup authorization, preserving existing configuration and using established defaults without repeated approval. Headless setup blocks when authority, material choices, conflicts, or required capabilities remain unresolved; setup permission never expands the **Mission envelope**
