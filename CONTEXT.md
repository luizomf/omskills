...
**Role inheritance**:
The default route for the standard dispatcher, coordinator, writer, and reviewer roles. They inherit the active provider, model, reasoning level, tools, and repository route unless an authorized caller explicitly overrides them. Role names delimit ownership, not intelligence or capability. Roles are relative to the assignment: a Ticket coordinator may be the conversational root or a dispatcher child; its writers and reviewers are non-delegating leaves. Absolute depth is not a role precondition. Actual tool capabilities and depth/child ceilings are harness-enforced, never established or overridden by role assertions.
