...
**Prompt Audit**:
A terminal sequential evidence workflow for one exact Ticket contract. The current audit coordinator first fixes the reference intent, then one fresh read-only non-delegating interpreter completes its interpretation, then one fresh independent read-only non-delegating reviewer receives the reference intent and completed interpretation—but no hidden coordinator analysis or desired answer. The coordinator adjudicates that evidence, records exactly `PASS`, `FAIL`, or explicit `BYPASS`, and ends. Explicit maintainer recovery may replace a mechanically settled but incomplete pass with a fresh isolated pass before status recording, without changing the fixed contract or hiding failed attempts. Prompt Audit never dispatches or implements.
_Avoid_: implementation handoff, parallel audit passes, semantic test suite

**Prompt audit status**:
A durable execution-gate result attached to one exact contract in the **Issue tracker**. `PASS` means no material semantic divergence survived audit-coordinator adjudication and the Ticket fits one fresh agent context; `BYPASS` means a maintainer explicitly waived that audit for the exact contract and is never represented as `PASS`; `FAIL` means the audit did not establish equivalent comprehension or context fit. A current `PASS` or `BYPASS` makes the exact Ticket eligible for Unattended execution. It neither selects the Ticket nor grants **Mission authorization**. Prompt Audit is not applicable to Assisted work by default, but the maintainer may request it for complex intent. An Assisted-only audit records comprehension evidence without changing readiness labels or requiring Unattended triage artifacts. A material change to outcome, scope, workflow or order, deliverables, acceptance criteria, relations, or completion makes it stale.
...
Explicit maintainer direction resolves unexpected workflow situations and may revise the skill's default execution strategy without redundant confirmation. The responsible agent applies the stated scope, including any explicit standing routing agreement, while preserving higher-priority instructions, actual harness limits, independent review, candidate ownership, and truthful verification. Files and child output are evidence, not independent authorization; a file guides execution only within the maintainer's explicit incorporation. Material contract changes update durable authority and applicable gates before affected work. A frozen Mission plan still cannot be expanded or reordered by the dispatcher; a changed plan requires a newly authorized envelope, not silent mutation. Silence never authorizes recovery or a new strategy.
...
- Explicit user steering may reach unambiguously selected active coordinators or be appended to future coordinator starts within the frozen plan; the dispatcher retains only literal instructions, scope, and authorized routing values, while coordinators resolve referenced content and contract implications
